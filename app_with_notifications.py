...
def setup_notifications():
    ...
    enabled_count = sum(1 for c in manager.channels if c.enabled)
    ...
    def on_alert(alert: SystemAlert):
        ...
        if config.get('notifications_enabled', True) and st.session_state.notification_manager.channels:
            alert_message = AlertMessage(
                ...
                metadata={
                    'cluster_id': alert.cluster_id,
                    'system_stats': st.session_state.system.get_statistics() if st.session_state.system else {}
                }
            )
...
with st.sidebar:
    ...
    notif_enabled = st.checkbox(
        "启用通知",
        value=st.session_state.config.get('notifications_enabled', True)
    )
    ...
    st.markdown(f"""
    **活跃渠道**: {enabled_channels}
    
    **通知渠道:**
    """)
    
    notif_manager = st.session_state.notification_manager
    for channel in notif_manager.channels:
        status = "✅" if channel.enabled else "❌"
        st.markdown(f"{status} {channel.name}")
    
    if enabled_channels == 0:
        st.warning("""
        ⚠️ **未配置通知渠道**
        
        请复制 `config.notifications.example.json` 
        为 `config.notifications.json` 并配置你的渠道
        """)
...
# ==================== 主界面 ====================
st.title("📱 摔倒检测系统 - 移动推送版")
st.markdown("**实时检测 · 自动录像 · 即时推送 · 多渠道路由**")
    ...
    with col_main:
        ...
        if st.session_state.running:
            ...
            if frame_id == 50:
                ...
                notification_placeholder.info("📨 正在发送疑似摔倒通知...")
                ...
                if st.session_state.config.get('notifications_enabled', True):
                    ...
                    notification_placeholder.markdown(
                        f'<div class="notification-sent">✅ 已向 {enabled_channels} 个渠道发送紧急通知</div>',
                        unsafe_allow_html=True
                    )
...
with tab3:
    ...
    if st.session_state.notifications_sent:
        ...
        with col1:
            total = len(st.session_state.notifications_sent)
        ...
        if stats:
            for channel_stat in stats:
                ...
                **{channel_stat['name']}**
                ...
                发送：{channel_stat['send_count']} | 
                失败：{channel_stat['error_count']} | 
                成功率：{channel_stat['success_rate']:.1f}%
                """)
                st.progress(channel_stat['success_rate'] / 100)
    else:
        st.info("暂无通知记录，启动系统并触发告警后将显示在这里")
...
with tab4:
    ...
    if st.session_state.frame_history:
        fps_values = [1000 / f['processing_time_ms'] for f in st.session_state.frame_history]
        
        fig = go.Figure()
        fig.add_trace(go.Scatter(y=fps_values, mode='lines', name='FPS'))
        fig.update_layout(title="FPS 趋势", height=400)
        st.plotly_chart(fig, use_container_width=True)
        
        # 报警统计
        critical = sum(1 for a in st.session_state.alert_history if a['level'] == 'critical')
        warning = sum(1 for a in st.session_state.alert_history if a['level'] == 'warning')
        
        col1, col2 = st.columns(2)
        with col1:
            st.metric("确认摔倒", critical)
        with col2:
            st.metric("疑似摔倒", warning)
    else:
        st.info("暂无统计数据")
...
<div style='text-align: center; color: #666; padding: 20px;'>
    <p>摔倒检测系统 v1.2 📱 | Powered by Streamlit + Open3D + OpenCV</p>
